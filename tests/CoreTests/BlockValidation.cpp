...
bool lift_up_difficulty(const CryptoNote::Currency& currency, std::vector<test_event_entry>& events,
  ...
  for (size_t i = 0; i < new_block_count; ++i) {
    ...
    // TODO (GCJ) fix and check this wj
    CryptoNote::Difficulty diffic = currency.nextDifficulty(0,0,timestamps, cummulative_difficulties);
...
bool gen_block_invalid_nonce::generate(std::vector<test_event_entry>& events) const {
  ...
  //TODO (GCJ) Fix and check
  // Create invalid nonce
  Difficulty diffic = m_currency->nextDifficulty(0,0,timestamps, commulative_difficulties);
  assert(1 < diffic);
...
bool gen_block_invalid_binary_format::generate(std::vector<test_event_entry>& events) const {
  ...
  do {
    ...
    // TODO (GCJ) Fix and Check
    diffic = m_currency->nextDifficulty(0,0,timestamps, cummulative_difficulties);
